...
The pre-generated table is in `/data/counterbalancing_table_v2.csv` (canonical source) and
copied to `/experiment/data/counterbalancing_table_v2.csv` for JATOS serving.
...
/data/                        — CSV outputs (do not delete existing files, only add)
  ...
  counterbalancing_table_v2.csv        — pre-generated trial assignment table (canonical source; 144 rows, no groups)
  results/                          — raw JATOS result folders for approved/main experiment data
    combined_raw.csv                — output of load_data.py
    demographics.csv                — output of load_data.py (one row per participant)
    test_trials.csv                 — output of preprocess.py
    learning_trials.csv             — output of preprocess.py
  to_review/                        — JATOS result folders pending Prolific approval/rejection
    combined_raw.csv                — same format as results/; generated by run_review.sh
    demographics.csv
    test_trials.csv
    learning_trials.csv
...
/data_analysis/               — Python and R analysis scripts (renamed from /planning/)
  scripts/
    load_data.py              — reads JATOS result folders → combined_raw.csv + demographics.csv
    preprocess.py             — cleans types, derives correct/confidence_z → test/learning CSVs
    exploration.py            — exploratory plots; supports --data-dir flag for to_review/
    run_review.sh             — runs full pipeline on data/to_review/ without touching data/results/
    analyze_results.R         — main R analysis (exclusions, descriptives, LMEMs)
...
2. **Trial sequence is loaded from CSV, not hardcoded.** The experiment reads
   `/experiment/data/counterbalancing_table_v2.csv` at runtime. No group filtering — all
...
**Practice stimuli** (nodes I, J, K) have been finalised and are defined in
`CONFIG.practiceStimuli`: `fractal9_A.png` (I), `fractal2_S.png` (J), `fractal22_S.png` (K),
stored in `experiment/assets/practice/`. `getStimulusPath()` looks these up from
`CONFIG.practiceStimuli` for nodes I–K.
...
4. **Practice** — shortened version of the main task (fewer learning steps, fewer
   test questions); gives feedback on cover task; **practice data is not saved to JATOS**
...
| **Attention fail** | Misses 3+ consecutive trials or ≥25% in a block (learning or test phase) | `EARLY_EXIT_ATTENTION` | `prolificAttentionExitURL` | Awaiting review (manual) | Prorated (manual) |
| **Missing PID** | Arrived without Prolific URL params and not a preview/admin worker | — | — | Abort (`jatos.abortStudy`), no redirect | None |

The three completion codes and their Prolific redirect URLs are set in `config.js`:

| Exit | Code | URL |
|------|------|-----|
| Full completion | `C104P3H9` | `https://app.prolific.com/submissions/complete?cc=C104P3H9` |
| Screen-out | `C11G0DN3` | `https://app.prolific.com/submissions/complete?cc=C11G0DN3` |
| Attention fail | `C10213M3` | `https://app.prolific.com/submissions/complete?cc=C10213M3` |
...
- Maximum response time is **4000 ms** (configurable via `testMaxResponseTime`)
...
| Learning | `maxMissRatePerBlock`   | 0.25    | End of block only | ≥25 % of trials missed in the completed block |
| Test     | `maxConsecutiveMisses`  | 3       | After every trial | N unanswered trials in a row |
| Test     | `maxMissRatePerBlock`   | 0.25    | End of block only | ≥25 % of trials missed in the completed block |
...
### Analysis-stage exclusions (post-hoc, in `analyze_results.R`)

Applied after data collection to the preprocessed CSVs. These are separate from payment —
all participants are paid regardless of whether they pass these checks.

All criteria are applied together: any participant flagged by **any** criterion is excluded
from all three dataframes (learning, testing, demographics).

| Source | Criterion | Threshold | Variable | Notes |
|--------|-----------|-----------|----------|-------|
| `learning_trials` | ≥90% same cover-task response | `n ≥ total_learning_trials × 0.9` | `cover_response` | Detects button-mashing (e.g. always pressing F) |
| `learning_trials` | ≥90% cover-task RT below 200 ms | `prop_fast ≥ 0.9` | `cover_rt` | Fast responses inconsistent with reading the stimulus |
| `learning_trials` | Cover-task RT CV < 0.1 | `sd/mean < 0.1` | `cover_rt` | Near-constant RT indicates disengagement |
| `test_trials` | ≥90% same chosen position | `n ≥ total_test_trials × 0.9` | `chosen_position` | Always picking left or right |
| `test_trials` | ≥90% 2AFC RT below 300 ms | `prop_fast ≥ 0.9` | `rt` | Fast responses inconsistent with deliberation |
| `test_trials` | 2AFC RT CV < 0.1 | `sd/mean < 0.1` | `rt` | Near-constant RT |
| `test_trials` | ≥90% confidence RT below 300 ms | `prop_fast ≥ 0.9` | `confidence_rt` | Fast slider movement without consideration |
| `test_trials` | Confidence RT CV < 0.1 | `sd/mean < 0.1` | `confidence_rt` | Near-constant RT |
| `test_trials` | Confidence response CV < 0.1 | `sd/mean < 0.1` | `confidence_response` | Always using same slider position |
| `test_trials` | Slider start–response correlation ≥ 0.9 | `r ≥ 0.9` | `confidence_slider_start` vs `confidence_response` | Never moving the slider from its start position |

**Total learning trials** = `blocks × walk_length` = 4 × 48 = 192.
**Total test trials** = `blocks × questions_per_block` = 4 × 36 = 144.
...
| `cover_correct` | **Derived by preprocess.py.** 1 if response matched the correct symmetry judgement, 0 if wrong or timed out (same accuracy convention as `correct` — missed trials count as incorrect). |
...
| `chose_plausible` | Boolean — whether the chosen option is the plausible one; `null` only on timeout. For T0 always `false` (both options implausible), for T2 always `true` (both plausible), for T1 reflects actual choice. |
| `response` | Button index (`0` = left, `1` = right) from `jsPsychHtmlButtonResponse`, or `null` if timed out. Not a keypress — the test phase uses clickable image buttons. |
...
| `correct` | **Derived by preprocess.py.** For T1 trials: 1 if participant chose the plausible option, 0 if not or timed out. `NaN` for T0/T2 (no correct answer defined). Accuracy = `mean(correct)` on T1 rows = correct / all_T1_trials (timeouts count as incorrect, per supervisor convention). |
...
- `Utils.getStimulusPath(node)` uses `jatos.studyAssetsUrl` (absolute URL) when running
  ...
  `studySessionData.stimulus_map`; for practice nodes (I–K) it looks up from
  `CONFIG.practiceStimuli`. Because `studySessionData` resets on page reload locally, the
  main node stimulus map won't be present when opening a mid-study component directly —
  those images will fall back gracefully, which is expected behaviour during development.
...
| `practiceWalkLength` | 5 | Short walk for practice; adjust in config to change |
| `testMaxResponseTime` | 4000 ms | Max time to respond in 2AFC; record timeout if no response |
...
2. **Comprehension check (2AFC)** — up to 2 attempts; if participant failed the cover
   check AND fails this one, the comprehension fail exit is triggered; otherwise correct
   answer is revealed and they continue. Both comprehension checks run before any practice trials.
3. **Untimed cover task practice** — deterministic walk; feedback after each step.
4. **Timed cover task practice** — same walk, `stimulusDuration` ms limit; visual
   warning if no response given within the time limit (no audio).
5. **Untimed 2AFC practice** — `CONFIG.practiceTwoAFCTrials`; no correctness feedback (only timeout warning).
6. **Timed 2AFC practice** — same, `testMaxResponseTime` ms limit; timeout warning only, no correctness feedback.
...
- Practice stimuli use nodes I, J, K; images are looked up from `CONFIG.practiceStimuli` (finalised fractals in `assets/practice/`).
...
- The counterbalancing table is bundled inside the experiment directory at
  `/experiment/data/counterbalancing_table_v2.csv` and loaded at runtime via `fetch()`
  at the start of the main task component
- The canonical source is `/data/counterbalancing_table_v2.csv` (project root). If that
...
### Keypress feedback (learning phase only)
- **Sound**: `Utils.playKeyTone()` — 660 Hz sine wave, 50 ms, soft fade-out
- **Visual**: the label of the pressed key (`F — Symmetric` / `J — Not symmetric`) turns
  blue (`#2980b9`); the other label dims to grey (`#aaaaaa`). Green (`#2ECC71`) is reserved
  for correct-answer feedback only. Implemented via IDs `cover-label-sym` / `cover-label-notsym`.

The **test phase uses `jsPsychHtmlButtonResponse`** (clickable image buttons), not keypresses.
There is no keydown listener or label-highlighting in the test phase — button clicks are
handled natively by jsPsych. `data.response` in test trials is `0` (left) or `1` (right).
...
All previously open design questions have been resolved. Response keys for the cover task
and 2AFC are defined in `config.js` (`f`/`j` for both).
